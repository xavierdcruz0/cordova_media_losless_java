...
//    private static int MEDIA_ERR_NONE_SUPPORTED = 4;
    ...
    private STring tempWav = null;          // Temporary store for the WAV version of the above PCM file
    private int bufferSize = 1024;          // The number of elements to store in the buffer
    private int bytesPerBufferElement = 2;  // i.e. a short
    private int SAMPLE_RATE = 22050;        // Could change this dynamically later...
    ...
    public AudioPlayer(AudioHandler handler, String id, String file) {
        ...
        */
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            this.tempWav = Environment.getExternalStorageDirectory().getAbsolutePath() + "/tmprecording.WAV";
        } else {
            this.tempWav = "/data/data/" + handler.cordova.getActivity().getPackageName() + "/cache/tmprecording.WAV";
        }
    ...
    */
    ...
    public void moveFile(String file) {
        ...
        File f = new File(this.tempWav); // the temporary input audio, after having been converted to WAV
        ...
        String logMsg = "renaming " + this.tempWav + " to " + file;
    ...
    public void stopRecording() {
        if (this.recorder != null) {
            try{
                ...
                // create WAV container for .PCM file
                File rawPCM = new File(tempFile);
                File wavFile = new File(tempWav);
                WavEncoder wavEncoder = new WavEncoder();
                // takes the raw PCM File, an empty File object; converts the former to WAV, saves it in the latter location
                wavEncoder.rawToWav(rawPCM, wavFile);

                // original part of the method - allows user to access the audioFile, rather than the temporary audio
                // files that are used locally in this Class
                this.moveFile(this.audioFile);
            }
            catch (Exception e) {
                e.printStackTrace();
